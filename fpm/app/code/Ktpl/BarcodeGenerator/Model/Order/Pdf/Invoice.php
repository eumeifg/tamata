 ...
 */
class Invoice extends \Magedelight\Sales\Model\Order\Pdf\Invoice
{
    /**
    ...
    public function __construct(
        ...
        parent::__construct($paymentData, $string, $scopeConfig, $filesystem, $pdfConfig, $pdfTotalFactory,
            $pdfItemsFactory, $localeDate, $inlineTranslation, $addressRenderer, $storeManager, $localeResolver, $vendorHelper, $data);
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true, $invoice = null)
    {
        ...
        $this->_setFontRegular($page, 10);
        if ($putOrderId) {
            $page->drawText(__('Order ID # ') . $order->getRealOrderId().'-'.$invoice->getVendorOrderId(), 420, $top -= 10, 'UTF-8');
            $top +=15;
        }
        ...
        /* Billing Address */
        $billingAddress = $this->_formatAddress($this->addressRenderer->format($order->getShippingAddress(), 'pdf'));
        ...
        $page->drawText(__('Shipping address:'), 35, $top + 40, 'UTF-8');
