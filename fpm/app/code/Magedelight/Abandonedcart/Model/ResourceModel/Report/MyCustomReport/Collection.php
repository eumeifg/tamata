...
namespace Magedelight\Abandonedcart\Model\ResourceModel\Report\MyCustomReport;
...
class Collection extends \Magento\Sales\Model\ResourceModel\Report\Collection\AbstractCollection
{
    ...
    protected $tableForPeriod = [
        'daily'   => 'md_abandonedcart_report_daily',
        'monthly' => 'md_abandonedcart_report_monthly',
        'yearly'  => 'md_abandonedcart_report_yearly',
    ];
    ...
    protected function _applyAggregatedTable()
    {
        ...
        if (!$this->isTotals()) {
            $select->group(['period', 'product_id' ,'quote_id']);
        }
    ...
    /*public function applyAdditionalFilters($select = null)
    {
        ...
        //$select->where('email IN (?)', "roni_cost@example.com");
        //$select->where('product_name IN (?)', "Compete Track Tote");
        $select->where('email IN (?)', $this->getEmail());
        return $this;
    }
