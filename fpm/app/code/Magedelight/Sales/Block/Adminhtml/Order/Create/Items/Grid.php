 ...
 */
namespace Magedelight\Sales\Block\Adminhtml\Order\Create\Items;
...
class Grid extends \Magento\Sales\Block\Adminhtml\Order\Create\AbstractCreate
{
    ...
    public function getConfigureButtonHtml($item)
    {
        ...
        return $this->getLayout()
            ->createBlock(\Magento\Backend\Block\Widget\Button::class)->setData($options)->toHtml();
    }
    ...
    public function getProductVendors($_item)
    {
        if($_item->getProductType() == 'configurable'){
            $productId = $_item->getOptionByCode('simple_product')->getValue();
        }else{
            $productId = $_item->getProductId();
        }

        $collection = $this->_vendorProductFactory->create()->getAvailableVendorsForProduct($productId);
        
        $vendorCollections = [];
        
        if (!empty($collection)) {
            $vendorCollections = $collection->getData();
        }
        
        $vendors = [];
        
        foreach ($vendorCollections as $vendorCollection) {
