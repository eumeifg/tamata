...
class ExpireBalance
{
    ...
    {
        if (!$this->cronManagement->isLocked(Flag::AW_RAF_EXPIRE_ADVOCATE_BALANCE_LAST_EXEC_TIME)) {
            try {
                $this->advocateExpirationManagement->expireBalance();
            } catch (\LogicException $e) {
                $this->logger->error($e);
            }
            $this->cronManagement->setFlagData(Flag::AW_RAF_EXPIRE_ADVOCATE_BALANCE_LAST_EXEC_TIME);
        }
    }
}
