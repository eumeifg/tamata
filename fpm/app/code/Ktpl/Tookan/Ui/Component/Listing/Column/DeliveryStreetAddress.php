...
class DeliveryStreetAddress extends Column
{
        ...
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                /*if ($item["tookan_status"] == TookanStatus::READY_TO_SHIPPED) {
                    $item[$this->getData('name')] = $this->tookanHelper->getStoreStreetAddress();
                } else {*/
                    $order = $this->orderRepository->get($item["order_id"]);
                    $shippingAddress = $order->getShippingAddress();
                    $item[$this->getData('name')] = $shippingAddress->getStreet();
                /*}*/
            }
        }
