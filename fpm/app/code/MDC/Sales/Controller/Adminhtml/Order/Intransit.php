...
class Intransit extends \Magento\Backend\App\Action
{
    /**
    ...
    public function execute()
    {
        ...
        if ($vendorOrder->getId()) {
            try {
                $vendorOrder->setData("status", VendorOrder::STATUS_IN_TRANSIT)
                    ->save();
                $this->messageManager->addSuccess(
                        __('Vendor order status changed to In Transit.')
                );
                $this->_eventManager->dispatch('vendor_orders_in_transit_after', ['vendor_order' => $vendorOrder]);
            } catch (\Exception $e) {
                $this->_logger->critical($e);
                $this->messageManager->addError(__('Vendor order status cannot be changed to In Transit'));
            }
        }
