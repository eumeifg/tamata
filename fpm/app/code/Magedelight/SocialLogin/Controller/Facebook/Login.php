...
namespace Magedelight\SocialLogin\Controller\Facebook;
...
class Login extends Action
{
     ...
     * @param Facebookhelper  $helperFacebook
     */
    ...
    public function execute()
    {
        ...
        $facebookService = $this->oauthservicefactory->createService('facebook', $credentials, $this->storage, ['email','public_profile']);
        if (!empty($req_code)) {
            // retrieve the CSRF state parameter
            $state = isset($req_state) ? $req_state : null;
            // This was a callback request from facebook, get the token
            $token = $facebookService->requestAccessToken($req_code, $state);
            $result = json_decode($facebookService->request('/me?fields=email,first_name,last_name'), true);
    ...
    private function processResult($result)
    {
        ...
        if (!isset($result['id']) || isset($result['error_reason']) == 'user_denied') {
            ...
            $customerId = $this->helper->getCustomerExist($result['id'], 'facebook');
            if (!empty($customerId)) {
                ...
                $data = ['firstname' => $result['first_name'], 'lastname' => $result['last_name'], 'email' => $email,
                    'store_id' => $store_id, 'website_id' => $website_id, 'social_type' => 'facebook',
                ...
                if ($flag) {
                    ...
                    return $this->getResponse()->setBody('<script type="text/javascript">
                    try{window.opener.location.href="'.$customRedirectUrl.'";}catch(e)
