...
class Code extends \Magento\Framework\View\Element\Template
{
     ...
     * @throws \Magento\Framework\Exception\NoSuchEntityException
     */
    public function getAddressSaveEvent()
    {
        $session = $this->fbPixelSession->create();
        $addressSave = 404;
        if ($this->helper->isAddressSave()
            && $session->hasAddressSave()) {
            $addressSave = $this->helper->getPixelHtml($session->getAddressSave());
        }
        return $addressSave;
    }
     ...
     * @throws \Magento\Framework\Exception\NoSuchEntityException
     */
    public function getBuyNowEvent()
    {
        $session = $this->fbPixelSession->create();
        $buyNow = 404;
        if ($this->helper->isBuyNow()
            && $session->hasBuyNow()) {
            $buyNow = $this->helper->getPixelHtml($session->getBuyNow());
        }
        return $buyNow;
    }
    ...
    public function getFacebookEvent()
    {
        $session = $this->fbPixelSession->create();
        $facebook = 404;
        if ($this->helper->isSocialLogin()
            && $session->hasFacebook()) {
            $facebook = $this->helper->getPixelHtml($session->getFacebook());
        }
        return $facebook;
    }
    ...
    public function getGoogleEvent()
    {
        ...
        if ($this->helper->isSocialLogin()
            && $session->hasGoogle()) {
            $google = $this->helper->getPixelHtml($session->getGoogle());
        }
        return $google;
    }
