...
namespace Magedelight\VendorPromotion\Plugin\Helper;
...
class Data
{
    ...
    public function afterIsEnabled($subject, $result)
    {
        if ($result) {
            ...
            $selectedWebsites = $this->scopeConfig->getValue('vendorpromotion/general/select_website');
            $websites = explode(',', $selectedWebsites);
            if (in_array($domain, $websites) && $this->scopeConfig->getValue('vendorpromotion/license/data')) {
                $result = true;
            } else {
                $result = false;
            }
