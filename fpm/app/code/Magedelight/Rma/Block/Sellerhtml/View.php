...
class View extends \Magento\Framework\View\Element\Template
{
    ...
    {
        parent::_construct();
        if (!$this->_coreRegistry->registry('vendor_current_rma')) {
            return;
        }
        $this->setTemplate('view.phtml');

        $this->setRma($this->_coreRegistry->registry('vendor_current_rma'));
    ...
    {
        return $this->getUrl('mdrma/vendorrequest/index', ['tab' => '6,0']);
    }
    ...
    /* Magento get require status which will be perorm from vendor panel */
    public function getAllRmaStatus()
    {
        $allRmaStatus = [];
        $vendorRmaStatus = $this->_coreRegistry->registry('vendor_current_rma')->getStatus();
        switch ($vendorRmaStatus) {
            case "pending":
                $allRmaStatus['pending'] = __("Pending");
                $allRmaStatus['authorized'] = __("Authorized");
                $allRmaStatus['denied'] = __("Denied");
                break;
            case "authorized":
                $allRmaStatus['received'] = __("Return Received");
                break;
            case "received":
                $allRmaStatus['approved'] = __("Approved");
                $allRmaStatus['rejected'] = __("Rejected");
                break;
        }
        return $allRmaStatus;
    }
     ...
     */
    public function getVendorOrder($orderId)
    {
        return $this->vendorOrderRepository->getByOriginalOrderId(
            $orderId,
