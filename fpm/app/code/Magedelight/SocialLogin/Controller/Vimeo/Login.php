...
namespace Magedelight\SocialLogin\Controller\Vimeo;
...
class Login extends Action
{
    ...
    public function execute()
    {
        ...
        'consumerId' => $this->helper->getConsumerId('vimeo'),
        'consumerSecret' => $this->helper->getConsumerSecret('vimeo'),
        'callbackUrl' => $this->helper->getAuthUrl('vimeo')
        ]);

        $vimeoService = $this->oauthservicefactory->createService(
            'Vimeo',
            $credentials,
            $this->storage,
        ...
        if (!empty($req_code)) {
            ...
            $token = $vimeoService->requestAccessToken($req_code, $state);
            // Send a request now that we have access token
            $result = json_decode($vimeoService->request('/me'));
            // Show some of the resultant data

            $user = explode('/', $result->uri);
            $name = explode(' ', $result->name);

            $vimeodata = ['id' => $user[2], 'firstname' => $name[0], 'lastname' => $name[1]];
            $this->processResult($vimeodata);
    ...
    private function processResult($result)
    {
        if (!isset($result['id'])) {
            ...
            $customerId = $this->helper->getCustomerExist($result['id'], 'vimeo');
            if (!empty($customerId)) {
                ...
                if (!isset($result['lastname'])) {
                    $lastname = $result['id'].'_lastname';
                } else {
                    $lastname = $result['lastname'];
                }
                if (!isset($result['email'])) {
                    $email = $result['firstname'].'_'.$lastname.'@vimeo.com';
                    $flag = 1;
                } else {
                    $email = $result['email'];
                    $flag = 0;
                ...
                $data = ['firstname' => $result['firstname'], 'lastname' => $lastname, 'email' => $email,
