...
class Mddata extends AbstractHelper 
{
    ...
    {
        $extensionKey = "ek-magento2-social-media-login";
        return $extensionKey;
    }
    ...
    public function getAllowedDomainsCollection() 
    {
        ...
        $serial = $this->scopeConfig->getValue('sociallogin/license/serial_key', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $activation = $this->scopeConfig->getValue('sociallogin/license/activation_key', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $isEnabled = $this->scopeConfig->getValue('sociallogin/general/enable', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $selectedWebsites = $this->scopeConfig->getValue('sociallogin/general/select_website', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        if (strpos($url, 'localhost') === false && strpos($url, '127.0.0.1') === false) 
        {
            if ($serial == '' && $activation == '') {
                $this->messageManager->addError("Serial and Activation keys not found.Please enter valid keys for 'Social Media Login' extension.");
            }
            if ($activation == '') {
                $this->messageManager->addError("Activation key not found.Please enter valid activation key for 'Social Media Login' extension.");
            }

            if ($serial == '') {
                $this->messageManager->addError("Serial key not found.Please enter valid serial key for 'Social Media Login' extension.");
            }
            ...
            $keys['ak'] = $activation;
            $keys['domain'] = $domain;
            $keys['product_name'] = 'Social Media Login';
            ...
            try {
                $this->configWritter->save('sociallogin/license/data', 1);
                ...
                if( is_object(json_decode($response)) && 
                    ...
                    $this->configWritter->save('sociallogin/license/data', 0);
                    ...
                    if(!isset($mappedDomains['domains'])){
                        $this->configWritter->save('sociallogin/general/enable', 0);
                    }
                    ...
                    if(isset($mappedDomains['domains']))
                    {
                        $post = get_object_vars($this->request->getPost());
                                ...
                                !empty($post['groups']['general']['fields']['select_website']['value']))
                            {
                                ...
                                if(sizeof($websites) > 0 && !empty($websites[0]))
                                {
                                    ...
                                    $this->configWritter->save('sociallogin/general/select_website', $updateSelected);
                                    if(empty($updateSelected)){
                                        $this->configWritter->save('sociallogin/general/enable', 0);
                                        $this->configWritter->save('sociallogin/license/data', 0); 
                                        $this->configWritter->save('sociallogin/general/select_website', ''); 
                                    }
                                }
                            }
                        else{
                            if(!empty($selectedWebsites))
                            {
                                ...
                                $this->configWritter->save('sociallogin/general/select_website', $updateSelected2);
                        ...
                        if(empty($mappedDomains['domains']))
                        {
                            $this->configWritter->save('sociallogin/license/data', 0);   
                            $this->configWritter->save('sociallogin/general/enable', 0);
                            $this->configWritter->save('sociallogin/license/serial_key', '');
                            $this->configWritter->save('sociallogin/license/activation_key', '');
                            $this->configWritter->save('sociallogin/general/select_website', '');

                            $this->messageManager->addError('Invalid activation and serial key for "Social Media Login".');
                        }else{
                            $this->configWritter->save('sociallogin/license/data', 1);
                        }
                                ...
                                foreach($website->getStores() as $store)
                                {
                                       ...
                                       !in_array($maindomain, $mappedDomains['domains']))
                                    {
                                        $this->_configInterface
                                            ->saveConfig('sociallogin/general/enable', 0, 'websites', $website->getId());
                                        $this->_configInterface
                                            ->saveConfig('sociallogin/general/enable', 0, 'stores', $store->getId());
                                    }
                                    ...
                                    if((in_array($websiteUrl, $selected) || in_array($maindomain, $selected)))
                                    {
                                        if(isset($post['groups']['general']['fields']['select_website']))
                                        {
                                            if(isset($post['groups']['general']['fields']['enable']['value']))
                                            {
                                                ...
                                                {
                                                    $this->_configInterface
                                                        ->saveConfig('sociallogin/general/enable', 1, 'websites', $website->getId());
                                                    $this->_configInterface
                                                        ->saveConfig('sociallogin/general/enable', 1, 'stores', $store->getId());
                                                }else{
                                                    $this->_configInterface
                                                        ->saveConfig('sociallogin/general/enable', 0, 'websites', $website->getId());
                                                    $this->_configInterface
