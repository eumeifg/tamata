...
namespace Magedelight\SocialLogin\Controller\OpenId;
...
class LightOpenID
{
    ...
    public function __construct($host, $proxy = null, $customrequest, $request)
    {
        $this->set_realm($host);
        $this->set_proxy($proxy);
       
        $uri = rtrim(preg_replace('#((?<=\?)|&)openid\.[^&]+#', '', $request->getServer('REQUEST_URI')), '?');
        $this->returnUrl = $this->trustRoot.$uri;
        $this->data = $customrequest->getRequest()->getParams();
    ...
    protected function get_realm_protocol()
    {
        ...
        if (!empty($request->getServer('HTTPS'))) {
            $use_secure_protocol = ($request->getServer('HTTPS') != 'off');
        } elseif ($request->getServer('HTTP_X_FORWARDED_PROTO') !== null) {
            $use_secure_protocol = ($request->getServer('HTTP_X_FORWARDED_PROTO') == 'https');
        } elseif ($request->getServer('HTTP__WSSC')!== null) {
            $use_secure_protocol = ($request->getServer('HTTP__WSSC') == 'https');
        } else {
    ...
    protected function request($url, $method = 'GET', $params = [], $update_claimed_id = false)
    {
        ...
        return
                $use_curl ? $this->request_curl($url, $method, $params, $update_claimed_id) : $this->request_streams($url, $method, $params, $update_claimed_id);
    }
    ...
    protected function get_provider_name($provider_url)
    {
        ...
        if (!empty($provider_url)) {
            ...
            $result = strtolower(
                (count($tokens) > 1 && strlen($tokens[1]) > 3) ? $tokens[1] : (count($tokens) > 2 ? $tokens[2] : '')
            );
        }
