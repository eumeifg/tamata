 ...
 */
namespace Magedelight\Rma\Model\Rma\Status;
 ...
 */
class History extends \Magento\Rma\Model\Rma\Status\History implements CommentInterface
{
    /**#@+
    ...
    protected function _sendCommentEmail(
        ...
        if (intval($this->getVendorId()) !== 0) {
            $isVendor = true;
            // Add Customer
            $customerName = $order->getCustomerName();
            $customerEmail = $order->getCustomerEmail();
            $sendTo[] = ['email' => $customerEmail, 'name' => $customerName];
        } else {
            // Add Vendor
            $vendorObj = $this->vendorFactory->create()->load($rma->getVendorId());
            $vendorName = $vendorObj->getBusinessName();
            $vendorEmail = $vendorObj->getEmail();
            $sendTo[] = ['email' => $vendorEmail, 'name' => $vendorName];
        }
        ...
        foreach ($sendTo as $recipient) {
            $transport = $this->_transportBuilder->setTemplateIdentifier($template)
                ...
                ->setFromByScope($this->_rmaConfig->getIdentity())
    ...
    public function saveComment($comment, $visibleOnFrontend, $isAdmin = false)
    {
        ...
        // Adding Vendor Id and Vendor Name into table magento_rma_status_history.
        if ($this->getVendorId()) {
            $this->setVendorId($this->getVendorId());
        }
        if ($this->getVendorName()) {
            $this->setVendorName($this->getVendorName());
        }
        $this->save();
    }
    ...
    protected function _sendRmaEmailWithItems(Rma $rma, $rootConfig)
    {
        ...
        $vendorName = $vendorObj->getBusinessName();
        $vendorEmail = $vendorObj->getEmail();
        $sendTo[] = ['email' => $vendorEmail, 'name' => $vendorName, 'sendToVendor' => true];
            ...
            $transport = $this->_transportBuilder->setTemplateIdentifier($template)
                ...
                ->setTemplateVars(
                    [
                        'rma' => $rma,
                        'order' => $order,
                        'sendToVendor' => isset($recipient['sendToVendor']) ? true : false,
                        'store' => $store,
