...
class Mddata extends AbstractHelper
{
    ...
    public function getAllowedDomainsCollection()
    {
        ...
        $serial = $this->scopeConfig->getValue('vendorpromotion/license/serial_key', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $activation = $this->scopeConfig->getValue('vendorpromotion/license/activation_key', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $isEnabled = $this->scopeConfig->getValue('vendorpromotion/general/enable', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $selectedWebsites = $this->scopeConfig->getValue('vendorpromotion/general/select_website', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        if (strpos($url, 'localhost') === false && strpos($url, '127.0.0.1') === false) {
            ...
            try {
                $this->configWritter->save('vendorpromotion/license/data', 1);
                ...
                if (is_object(json_decode($response)) &&
                    ...
                    $this->configWritter->save('vendorpromotion/license/data', 0);
                    ...
                    if (!isset($mappedDomains['domains'])) {
                        $this->configWritter->save('vendorpromotion/general/enable', 0);
                    }
                    ...
                    if (isset($mappedDomains['domains'])) {
                        ...
                        if (isset($post['groups']['general']['fields']['select_website']['value'])) {
                            ...
                            if (sizeof($websites) > 0 && !empty($websites[0])) {
                                ...
                                $this->configWritter->save('vendorpromotion/general/select_website', $updateSelected);
                                if (empty($updateSelected)) {
                                    $this->configWritter->save('vendorpromotion/general/enable', 0);
                                    $this->configWritter->save('vendorpromotion/license/data', 0);
                                    $this->configWritter->save('vendorpromotion/general/select_website', '');
                                }
                            }
                        } else {
                            if (!empty($selectedWebsites)) {
                                ...
                                $this->configWritter->save('vendorpromotion/general/select_website', $updateSelected2);
                        ...
                        if (empty($mappedDomains['domains'])) {
                            ...
                            $this->configWritter->save('vendorpromotion/general/enable', 0);
                            $this->configWritter->save('vendorpromotion/license/serial_key', '');
                            $this->configWritter->save('vendorpromotion/license/activation_key', '');
                            $this->configWritter->save('vendorpromotion/general/select_website', '');
                                    ...
                                    if (!in_array($websiteUrl, $mappedDomains['domains']) &&
                                       !in_array($maindomain, $mappedDomains['domains'])) {
                                        $this->_configInterface
                                            ->saveConfig('vendorpromotion/general/enable', 0, 'websites', $website->getId());
                                    }
                                        ...
                                        if (isset($post['groups']['general']['fields']['select_website']['value'])) {
                                            if (isset($post['groups']['general']['fields']['enable']['value'])) {
                                                if ($post['groups']['general']['fields']['enable']['value']) {
                                                    $this->_configInterface
                                                        ->saveConfig('vendorpromotion/general/enable', 1, 'websites', $website->getId());
                                                    $this->_configInterface
                                                        ->saveConfig('vendorpromotion/general/enable', 1, 'stores', $store->getId());
                                                } else {
                                                    $this->_configInterface
                                                        ->saveConfig('vendorpromotion/general/enable', 0, 'websites', $website->getId());
                                                    $this->_configInterface
                                                        ->saveConfig('vendorpromotion/general/enable', 0, 'stores', $store->getId());
                                                }
                                            }
                                        }
                                    }
