...
namespace Magedelight\Abandonedcart\Plugin\Magento\Reports\Model\ResourceModel\Refresh;
    ...
    public function afterLoadData($subject, $result, $printQuery = false, $logQuery = false)
    {
        if (!count($this->_items)) {
            $data = [
                [
                    'id' => 'mycustomreport',
                    'report' => __('Abandoned Cart Reports'),
                    'comment' => __('Abandoned Cart Reports'),
                    'updated_at' => $this->_getUpdatedAt(\Magedelight\Abandonedcart\Model\Flag::
                     REPORT_MYCUSTOMREPORT_FLAG_CODE)
                ],
            ];
