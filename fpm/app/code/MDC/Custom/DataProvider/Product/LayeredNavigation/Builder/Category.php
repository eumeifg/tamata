...
namespace MDC\Custom\DataProvider\Product\LayeredNavigation\Builder;
 ...
 * @inheritdoc
 */
class Category extends \Magedelight\CatalogGraphQl\DataProvider\Product\LayeredNavigation\Builder\Category
{
    ...
    public function build(AggregationInterface $aggregation, ?int $storeId): array
    {
        ...
        $marketingCategory = \array_column(
            $this->attributesMapper->getAttributesValues(
                $this->resourceConnection->getConnection()->fetchAll(
                    $this->categoryAttributeQuery->getQuery($categoryIds, ['is_marketing'], $storeId)
                )
            ),
            'is_marketing',
            'entity_id');

        if(!empty($marketingCategory))
        { 
            foreach ($marketingCategory as $key => $value) {
                if($value == 1)
                {
                    $nonMarketingArray[] = $key;
                }
                
            }
        }

         if(!empty($nonMarketingArray))
         {
             $finalIds = \array_diff($categoryIds,$nonMarketingArray);
        ...
        $categoryIds = \array_diff($finalIds, [$this->rootCategoryProvider->getRootCategory($storeId)]);
