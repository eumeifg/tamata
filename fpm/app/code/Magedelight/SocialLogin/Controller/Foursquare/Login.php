...
namespace Magedelight\SocialLogin\Controller\Foursquare;
...
class Login extends Action
{
     ...
     * @param Foursquarehelper $Foursquarehelper
    ...
    public function execute()
    {
        ...
        'consumerId' => $this->helper->getConsumerId('foursquare'),
        'consumerSecret' => $this->helper->getConsumerSecret('foursquare'),
        'callbackUrl' => $this->helper->getAuthUrl('foursquare')
        ]);
        
        $foursquareService = $this->oauthservicefactory->createService('foursquare', $credentials, $this->storage);
        ...
        if (!empty($req_code)) {
            ...
            $foursquareService->requestAccessToken($req_code);
            // Send a request with it
            $result = json_decode($foursquareService->request('users/self'), true);
            // Show some of the resultant data
            $this->processResult($result['response']['user']);
    ...
    {
        if (!isset($result['id']) || isset($result['error_reason']) == 'user_denied') {
            ...
            $customerId = $this->helper->getCustomerExist($result['id'], 'foursquare');
            if (!empty($customerId)) {
                ...
                $flag = 0;
                if (!isset($result['contact']['email'])) {
                    $email = $result['firstName'].'_'.$result['lastName'].'@foursquare.com';
                    $flag = 1;
                } else {
                ...
                'store_id' => $store_id, 'website_id' => $website_id, 'social_type' => 'foursquare',
