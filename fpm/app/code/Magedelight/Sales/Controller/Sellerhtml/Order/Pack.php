 ...
 */
class Pack extends \Magedelight\Backend\App\Action
{
    /**
    ...
    public function execute()
    {
        ...
        if ($vendorOrder->getId()) {
            try {
                $vendorOrder->setData("status", Order::STATUS_PACKED)
                    ->save();
                $this->messageManager->addSuccess(
                    __('The order status changed to packed.')
                );
            } catch (\Exception $e) {
                $this->_logger->critical($e);
                $this->messageManager->addError(__('The order status cannot be changed to packed'));
            }
        }
