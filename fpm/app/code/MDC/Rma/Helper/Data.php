...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    function isReturnAllowed($_item)
    {
        ...
        if($canCreateRMA){
            ...
            if (
                strtolower($orderStatus) == VendorOrder::STATUS_COMPLETE &&
                $canRProduct
            ) {
                return true;
            }
    ...
    function checkTookanOrderComplete($_item)
    {
        $realOrderNumber = $_item->getOrder()->getShipmentsCollection()->getFirstItem();
        $shipmentIncrementId = $realOrderNumber->getIncrementId();

        $customTableData = $this->_modelStatusLogFactory->create()->getCollection()->addFieldToFilter(
                    'shipment_id',
                    ['eq' => $shipmentIncrementId]
                )->getFirstItem();

        $storeScope = \Magento\Store\Model\ScopeInterface::SCOPE_STORE;
        $returnPolicyValidity = $this->scopeConfig->getValue(self::XML_PATH_EMAIL_RECIPIENT, $storeScope);

        $orderCompletedDate = $customTableData->getCreatedAt();
        $currentDate        = strtotime($this->date->gmtDate());
        //$currentDate        = strtotime("2020-01-16 14:18:15");
        $daysAddedDate      = strtotime(date('Y-m-d H:i:s', strtotime($orderCompletedDate. ' + '.$returnPolicyValidity.' days')));

        if ($currentDate <= $daysAddedDate) {
            return true;
        } else {
            return false;
        }
